...
import {
    useGetProductsQuery, usePostProductsChangeNewMutation,
} from '../../../services/adminApi.jsx';

const NewProducts = () => {
    // 1) Fetch products
    const {data: getProducts, isLoading, error} = useGetProductsQuery();
    const [postProductsChangePopular] = usePostProductsChangeNewMutation();
    ...
    useEffect(() => {
        if (products.length && originalPopular.current.length === 0) {
            const popularIds = products.filter(p => p.isNew).map(p => p.id);
