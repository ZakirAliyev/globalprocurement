...
const PopularProducts = () => {
    ...
    const [postProductsChangePopular] = usePostProductsChangePopularMutation();

    const {data: getOrdersFilteredOrders} = useGetOrdersFilteredOrdersQuery()
    const orders = getOrdersFilteredOrders?.data
    ...
    useEffect(() => {
        if (products.length && originalPopular.current.length === 0) {
            const popularIds = products.filter(p => p.isPopular).map(p => p.id);
